...
namespace Aeag\AeagBundle\DependencyInjection;

use Aeag\AeagBundle\DependencyInjection\fpdf\PageGroup;
use Symfony\Bundle\FrameworkBundle\Controller\Controller;
use Symfony\Component\HttpFoundation\Response;

class PdfListeCommune extends PageGroup {

   function Header($departement) {
        ...
        $this->Cell(100);
        // Titre
        $this->Cell(1, 12, 'Liste des commune du département ' . $departement->getDept() . ' ' . $departement->getLibelle() , 0, 1, 'C');
        $this->Cell(100);
...
// Tableau coloré
    function Formatage($departement, $communes) {
        ...
        foreach ($communes as $entity) {
            $this->Cell($w[0], 4, $entity->getCommune(), 'LR', 0, 'C', $fill);
            $this->Cell($w[1], 4, $entity->getLibelle(), 'LR', 0, 'L', $fill);
            $this->Ln();
            $h = $h + 4;
            $fill = !$fill;
            $nbLignes += 1;
