...
class PdfListeProducteurNonPlafonneAll extends PageGroup {
    ...
    function Header($entity) {
        ...
        $this->Cell(100);
        // Titre
        $this->Cell(1, 12, 'Liste des producteurs non plafonnés', 0, 1, 'C');
        $this->Cell(100);
...
// Tableau coloré
    function Formatage($entities) {
        ...
        $w = array(25, 150, 15);
        $col = array();
        $header = array('Siret', 'Raison sociale', 'Aidable');
        // En-tête
        ...
        foreach ($entities as $entity) {
             ...
             if ($entity['Correspondant']){
                $this->Cell($w[1], 4, $entity['Correspondant']->getAdr1() . ' ' . $entity['Correspondant']->getAdr2() , 'LR', 0, 'L', $fill);
             }else {
                $this->Cell($w[1], 4, $entity[0]->getLibelle(), 'LR', 0, 'L', $fill);
             }
             $this->Cell($w[2], 4, $entity[0]->getAidable(), 'LR', 0, 'C', $fill);
            $this->ln();
            $h = $h + 4;
            $fill = !$fill;
            $nbLignes += 1;
