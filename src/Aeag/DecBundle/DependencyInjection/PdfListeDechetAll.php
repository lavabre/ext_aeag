...
class PdfListeDechetAll extends PageGroup {
    ...
    function Header($entity) {
        ...
        $this->Cell(100);
        // Titre
        $this->Cell(1, 12, 'Liste des codes déchets', 0, 1, 'C');
        $this->Cell(100);
...
// Tableau coloré
    function Formatage($entities) {
        ...
        $w = array(15, 160, 15);
        $header = array('Code', 'Libellé', 'Aidable');
        // En-tête
        ...
        foreach ($entities as $entity) {
            $this->Cell($w[0], 4, $entity->getCode(), 'LR', 0, 'C', $fill);
            $this->Cell($w[1], 4, $entity->getLibelle(), 'LR', 0, 'L', $fill);
            $this->Cell($w[2], 4, $entity->getAidable(), 'LR', 0, 'C', $fill);
            $this->Ln();
            $h = $h + 4;
            $fill = !$fill;
            $nbLignes += 1;
